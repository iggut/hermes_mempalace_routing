...
def test_hermes_mcp_style_end_to_end_status_duplicate_add_search() -> None:
    """One-path smoke for Hermes MCP dict-args and JSON payload wrappers."""

    def mempalace_status(args: dict, **kwargs: object) -> str:
        import json

        assert args == {}
        return json.dumps({"structuredContent": {"ok": True, "version": "1.2.3"}})

    def mempalace_check_duplicate(args: dict, **kwargs: object) -> str:
        import json

        assert args["content"] == "new fact"
        return json.dumps({"result": {"duplicate": False}})

    def mempalace_add_drawer(args: dict, **kwargs: object) -> str:
        import json

        assert args["wing"] == "hermes"
        assert args["room"] == "project/hermes"
        assert args["content"] == "new fact"
        return json.dumps({"result": {"drawer_id": "drawer-new-9"}})

    def mempalace_search(args: dict, **kwargs: object) -> str:
        import json

        assert args["query"] == "new"
        return json.dumps(
            {
                "result": {
                    "results": [
    ...
    is_dup, matched = adapter.check_duplicate("new fact", "hermes", "project/hermes")
    assert is_dup is False
    assert matched is None
    drawer_id = adapter.add_drawer("new fact", "hermes", "project/hermes")
    assert drawer_id == "drawer-new-9"
    hits = adapter.search("new", wing="hermes", room="project/hermes")
    assert len(hits) == 1
    assert hits[0].content == "new fact"
    assert hits[0].metadata["source_file"] == "ticket-123"


def test_coerce_tool_return_handles_dict_wrappers_and_malformed_strings() -> None:
    def check_duplicate_dict_wrapped(content: str, wing: str, room: str, threshold: float = 0.92):
        return {"result": {"is_duplicate": True, "best_match_id": "d-7"}}

    def search_structured_content(query: str, wing: str | None = None, room: str | None = None, limit: int = 24):
        return {
            "structuredContent": {
                "results": [{"id": "drawer-a", "content": "verbatim", "wing": "w", "room": "project/w"}]
            }
        }

    def status_error_passthrough():
        return {"error": "unavailable"}

    def add_drawer_plain_string(content: str, wing: str, room: str, metadata: dict | None = None):
        return "drawer-plain-1"

    def search_malformed_json(query: str, wing: str | None = None, room: str | None = None, limit: int = 24):
        return '{"result": {"results": ['

    adapter = MemPalaceAdapter(
        {
            "mempalace_check_duplicate": check_duplicate_dict_wrapped,
            "mempalace_search": search_structured_content,
            "mempalace_status": status_error_passthrough,
            "mempalace_add_drawer": add_drawer_plain_string,
        }
    )
    is_dup, mid = adapter.check_duplicate("x", "w", "r")
    assert is_dup is True
    assert mid == "d-7"
    hits = adapter.search("x")
    assert len(hits) == 1
    assert hits[0].drawer_id == "drawer-a"
    status = adapter.status()
    assert status["ok"] is True
    assert status["version"] == ""
    assert adapter.add_drawer("x", "w", "r") == "drawer-plain-1"

    adapter_malformed = MemPalaceAdapter({"mempalace_search": search_malformed_json})
    assert adapter_malformed.search("x") == []
